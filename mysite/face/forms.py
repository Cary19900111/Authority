...
class LoginUserForm(forms.Form):
    username = forms.CharField(label='account',error_messages={'required':'account is not empty'},
                               widget=forms.TextInput(attrs={'class':'form-control'}))
    password = forms.CharField(label='password', error_messages={'required': 'account is not empty'},
                               widget=forms.PasswordInput(attrs={'class': 'form-control'}))
    def __init__(self,request=None,*args,**kwargs):
        self.request = request
        self.user_cache = None
        super(LoginUserForm,self).__init__(*args,**kwargs)
#Field子类的clean() 方法。它负责以正确的顺序运行to_python、validate 和 run_validators 并传播它们的错误。
#如果任何时刻、任何方法引发ValidationError，验证将停止并引发这个错误。这个方法返回验证后的数据，
#这个数据在后面将插入到表单的 cleaned_data 字典中。
#表单子类中的clean_<fieldname>() 方法 —— <fieldname> 通过表单中的字段名称替换
#自动验证第五步
    def clean_password(self):
        ...
        if username and password:
            ...
            if self.user_cache is None:
                raise forms.ValidationError("account not accodinate to  password")
#抛出多个错误
                ...
                raise forms.ValidationError("account is invalid")
